...
use App\Controller\ptCollectController;
...
{
    #[Route('/back/{idUser}', name: 'app_pt_collect_back_index', methods: ['GET'])]
    public function index1(PtCollectRepository $ptCollectRepository,UserRepository $repository , $idUser): Response
    {
    ...
    public function new1(Request $request, EntityManagerInterface $entityManager,UserRepository $repository , $idUser): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('app_pt_collect_back_index', ['idUser' => $user_verif->getId()], Response::HTTP_SEE_OTHER);
        }
    ...
    #[Route('/back/{id}/{idUser}', name: 'app_pt_collect_back_show', methods: ['GET'])]
    public function show1(PtCollect $ptCollect,UserRepository $repository , $idUser): Response
    {
        $user_verif = $repository->find($idUser);
        if (!$user_verif) {
            throw $this->createNotFoundException('User not found');
    ...
    #[Route('/back/{id}/edit/{idUser}', name: 'app_pt_collect_back_edit', methods: ['GET', 'POST'])]
    public function edit1(Request $request, PtCollect $ptCollect, EntityManagerInterface $entityManager,UserRepository $repository , $idUser): Response
    {
        $user_verif = $repository->find($idUser);
        if (!$user_verif) {
            throw $this->createNotFoundException('User not found');
    ...
    #[Route('/back/{id}/{idUser}', name: 'app_pt_collect_back_delete', methods: ['POST'])]
    public function delete1(Request $request, EntityManagerInterface $entityManager, UserRepository $repository, $id, $idUser): Response
    {
        ...
        $ptCollectRepository = $entityManager->getRepository(PtCollect::class);
        $ptCollect = $ptCollectRepository->find($id);

        if (!$ptCollect) {
            throw $this->createNotFoundException('Point of Collection not found');
        }
