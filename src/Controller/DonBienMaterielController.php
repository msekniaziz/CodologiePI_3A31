...
The Codologie Team";
    ...
    public function editback($id,Request $request, DonBienMateriel $donBienMateriel, EntityManagerInterface $entityManager): Response
    { 
        $userId =$donBienMateriel->getUserId() ;

        $user_verif = $entityManager->getRepository(User::class)->find($userId);

    //    $user_verif = $repository->find($idUser);
        if (!$user_verif) {
            throw $this->createNotFoundException('User not found');
        }
       // $check=$donBienMateriel->find($id);
        if (!$id) {
            ...
            // $entityManager->flush();
            $userId = $donBienMateriel->getUserId();
            $donActive = $entityManager->getRepository(DonBienMateriel::class)->find($id);

            if ($donActive != null) {
                $donActive->setStatutDon(true);
                $entityManager->flush();
                $nbUp = $user_verif->getNbPoints() + 1;
            ...
            $user_verif->setNbPoints($nbUp);
            $entityManager->persist($user_verif);
            $entityManager->flush();
