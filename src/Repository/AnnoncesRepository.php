...
//        ;
//    }
    public function countByCategoryHouse(): int
    {
        return $this->createQueryBuilder('a')
            ->select('count(a.id)')
            ->where('a.category = :category')
            ->setParameter('category', 'House')
            ->getQuery()
            ->getSingleScalarResult();
    }

// Compte pour la catégorie "Garden"
    public function countByCategoryGarden(): int
    {
        return $this->createQueryBuilder('a')
            ->select('count(a.id)')
            ->where('a.category = :category')
            ->setParameter('category','Garden')
            ->getQuery()
            ->getSingleScalarResult();
    }
}
